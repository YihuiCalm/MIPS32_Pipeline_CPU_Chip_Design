...
set basename MIPS32_CPU_top       ;# Top-level module name
set myClk clk                    ;# The name of your clock 
set virtual 0                       ;# 1 if virtual clock, 0 if real clock
set myPeriod_ns 20               ;# desired clock period (in ns) (sets speed goal)
...
####################################
set filename [format "%s%s%s" ./src/syn/ $filebase ".v"]
...
####################################
set filename [format "%s%s%s" ./src/syn/ $filebase ".sdf"]
...
####################################
set filename [format "%s%s%s" ./src/syn/ $filebase ".sdc"]
write_sdc $filename
...
# report check design
set filename [format "%s%s%s" ./reports/ $filebase ".check"]
redirect $filename { check_design }
...
#set memorys to don't touch cells
# set_dont_touch [list [get_cells inst_data_MEM]]
# set_dont_touch [list [get_cells inst_register]]
...
# Write out protocol
set filename [format "%s%s%s" ./src/syn/ $filebase ".spf"]
